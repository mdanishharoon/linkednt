...
export function maxTokensForMode(mode: Mode): number {
  // Headroom for reasoning models (qwen3 etc.). Even with reasoning_format:"hidden"
  // we want enough budget for the model to fully reason and emit the final answer.
  switch (mode) {
    case "strip":
      return 512;
    case "summarise":
